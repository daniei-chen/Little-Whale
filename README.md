...
西瓜视频 / 今日头条 / 好看视频 / 微视 / 豆瓣 / 百度贴吧 /
腾讯视频 / 爱奇艺 / 优酷 / 芒果TV / 虎牙 / 斗鱼

**为什么它们单独一组**：实测这些平台的网页**全是 SPA** ——
首页 HTML 只有几 KB 的空壳，一个视频地址、一张内容图都没有，
数据全靠 JS 渲染。直接 HTTP 抓页面完全走不通。

而这个 App 内置了一个隐藏 WebView —— 让页面**真的跑起来**，
再读渲染后的 DOM 就有内容了。加平台因此变成**填配置**：

```dart
GenericLocalPlatform(
  key: 'ixigua', name: '西瓜视频',
  hosts: ['ixigua.com'], referer: 'https://www.ixigua.com/',
),
```
...
> - **识别不了动图**
>
> **长视频平台拿不到正片**：优酷 / 爱奇艺 / 腾讯视频 / 芒果TV 的视频是
> **DRM 加密流**，页面里根本没有可下载的直链 —— 这是平台的内容保护，不是适配器的问题。
> （实测优酷能提取到相关推荐图，但提取不到正片。）
>
> **实测结果（用真实内容页验证）**：
>
> | 平台 | 结果 |
...
> | 好看视频 | ✅ 提取到视频地址 + 标题 |
> | 虎牙 | ✅ 提取到内容图 |
> | 豆瓣 | ❌ **反爬拦截** —— 连带真实浏览器指纹的 WebView 也被挡（Node 直连返回 403） |
> | 其余 9 个 | 尚未拿到真实分享链接，**只验证了「识别 + 渲染机制可用」** |
>
> 这不是含糊其辞：**通用适配器这条路本身是通的**（好看视频/虎牙已证明），
> 但每个平台的页面结构不同，需要各自的分享链接来逐个调优。

这是「覆盖广度」和「单平台质量」的取舍 —— 先保证能用，用的人多了再做专用优化。
...
**B站限流会自动重试**
B站对专栏/动态接口有频率风控（`-509` / `-352`），连着解析几篇就会撞上。

实测确认过两件事：
1. **限流是 IP 级的** —— 被拦时网页接口同样返回一张约 1.3 KB 的拦截页
   （正常页面 50 KB），所以「改走网页」这条路救不了重度限流
2. App 现在已经带上 **`buvid` 设备指纹**（来自公开接口 `x/frontend/finger/spi`），
   明显降低被判成机器人的概率

重试策略是**指数退避 2/4/8/16 秒**（合计约 30 秒）。对用户来说
「等一下能出结果」远好过「直接失败」；真到了还不行的地步，提示会
明确告诉用户**等半分钟再来**。
